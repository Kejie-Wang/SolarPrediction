...
class Similar_Day_Reader:
    ...
    def _get_day_tuple(self, time):
        time = np.reshape(time, [-1, 24, time.shape[-1]])
        time = time[:, 0, 0:3]
    ...
    def get_data(train_time, validation_time, test_time):
        ...
        return np.array([dic[x] for x in train_time]), np.array([dic[x] for x in validation_time]), np.array([dic[x] for x in test_time])
