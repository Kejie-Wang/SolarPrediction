...
def main(_):
    ...
    model = Model([x_ir, x_mete], y_, hour_index, keep_prob, config)
    ...
    with tf.Session(config=tensor_config) as sess:
        ...
        validation_feed = fill_feed_dict(x_ir, x_mete, hour_index, y_, keep_prob, validation_set, 1.0, modality)
        test_feed = fill_feed_dict(x_ir, x_mete, hour_index, y_, keep_prob, test_set, 1.0, modality)
        ...
        for i in range(epoch_size):
            ...
            if i%config.test_step == 0:
                ...
                if config.regressor == "mse" or config.regressor == "msvr" or config.regressor == "meef":
                    ...
                    #         print batch[-1].shape
                    #     train_feed = fill_feed_dict(x_ir, x_mete, hour_index, y_, keep_prob, batch, 1.0, modality)
                    ...
                    # coverage_rate = 0; num = 0; bs = 20
                    # for ptr in range(bs, len(train_set[-1]), bs):
                    #     batch = []
                    #     for j in range(len(train_set)):
                    #         train_set_level = []
                    #         for k in range(train_set[j]):
                    #             train_set_level.append(train_set[j][k][ptr-bs:ptr])
                    #         batch.append(train_set_level)
                    #     train_feed = fill_feed_dict(x_ir, x_mete, hour_index, y_, keep_prob, batch, 1.0, modality)
            ...
            feed_dict = fill_feed_dict(x_ir, x_mete, hour_index, y_, keep_prob, batch, 0.6, modality)
            sess.run(model.optimize, feed_dict=feed_dict)

            # print step
            if i%config.print_step == 0:
                feed_dict = fill_feed_dict(x_ir, x_mete, hour_index, y_, keep_prob, batch, 1.0, modality)
