...
model_path = '../../saved_wavedec_model/'
output_path = '../../output_wavedec_model/'

HOUR_IN_A_DAY = 24

def fill_feed_dict(x_ir_placeholder, \
                x_mete_placeholder, \
                x_similar_day_placeholder, \
    ...
    index = 0
    if modality[0] == 1:
        for i in range(len(x_ir_placeholder)):
            feed_dict[x_ir_placeholder[i]] = feed_data[index][i]
        index += 1
    if modality[1] == 1:
        for i in range(len(x_mete_placeholder)):
            feed_dict[x_mete_placeholder[i]] = feed_data[index][i]
        index += 1
    if modality[2] == 1:
        feed_dict[x_similar_day_placeholder] = feed_data[index]
        index += 1
...
def batch_test(sess, x_ir, x_mete, x_similar_day, hour_index, y_, keep_prob, modality, evaluation, get_data_set, batch_size):
    ptr=0
    results = []
    while True:
        ...
        if len(data_set) == 0:
            break
        feed_dict = fill_feed_dict(x_ir, x_mete, x_similar_day, hour_index, y_, keep_prob, data_set, 1.0, modality)
...
def main(_):
    ...
    x_ir = []
    x_mete = []
    n_step_level = reader.get_n_step_level()
    print n_step_level
    for n_step in n_step_level:
        if modality[0] == 1:
            x_ir.append(tf.placeholder(tf.float32, [None, n_step, n_input_ir]))
        if modality[1] == 1:
            x_mete.append(tf.placeholder(tf.float32, [None, n_step, n_input_mete]))

    x_similar_day = tf.placeholder(tf.float32, [None, HOUR_IN_A_DAY, 1])

    hour_index = tf.placeholder(tf.float32)
    keep_prob = tf.placeholder(tf.float32)
    y_ = tf.placeholder(tf.float32, [None, n_target])

    model = Model([x_ir, x_mete, x_similar_day], y_, hour_index, keep_prob, config)
    ...
    with tf.Session(config=tensor_config) as sess:
        ...
        validation_feed = fill_feed_dict(x_ir, x_mete, x_similar_day, hour_index, y_, keep_prob, validation_set, 1.0, modality)
        test_feed = fill_feed_dict(x_ir, x_mete, x_similar_day, hour_index, y_, keep_prob, test_set, 1.0, modality)
        ...
        for i in range(epoch_size):
            ...
            if i%config.test_step == 0:
                ...
                if config.regressor == "mse" or config.regressor == "msvr" or config.regressor == "meef":
                    test_result = batch_test(sess, x_ir, x_mete, x_similar_day, hour_index, y_, keep_prob, modality, model.prediction, reader.get_test_set, evaluation_batch_size)
                    rmse, mae = rmse_and_mae(reader.target_test_data, test_result)
                    print "Test  RMSE: ", rmse, "Test  MAE: ", mae

                    validation_result = batch_test(sess, x_ir, x_mete, x_similar_day, hour_index, y_, keep_prob, modality, model.prediction, reader.get_validation_set, evaluation_batch_size)
                    rmse, mae = rmse_and_mae(reader.target_validation_data, validation_result)
                    print "Valid RMSE: ", rmse, "Valid MAE: ", mae

                    train_result = batch_test(sess, x_ir, x_mete, x_similar_day, hour_index, y_, keep_prob, modality, model.prediction, reader.get_train_set, evaluation_batch_size)
            ...
            feed_dict = fill_feed_dict(x_ir, x_mete, x_similar_day, hour_index, y_, keep_prob, batch, 0.6, modality)
            sess.run(model.optimize, feed_dict=feed_dict)

            # print step
            if i%config.print_step == 0:
                feed_dict = fill_feed_dict(x_ir, x_mete, x_similar_day, hour_index, y_, keep_prob, batch, 1.0, modality)
            ...
            if i%10 ==0:
                if config.regressor == "mse" or config.regressor == "msvr" or config.regressor == "meef":
                    ...
                    if loss < validation_min:
                        print "min:", np.sqrt(loss)
                        validation_min = loss
                        best_test_result = batch_test(sess, x_ir, x_mete, x_similar_day, hour_index, y_, keep_prob, modality, model.prediction, reader.get_test_set, evaluation_batch_size)
