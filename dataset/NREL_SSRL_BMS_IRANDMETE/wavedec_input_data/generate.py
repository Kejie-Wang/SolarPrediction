...
target_hourly_data = np.loadtxt(data_dir + 'target_hourly_data.csv', delimiter=',')

data_length = len(ir_data)
validation_length = validation_day_length * 24 * 60 / MINUTES_TO_AVG
test_length = test_day_length * 24 * 60 / MINUTES_TO_AVG
...
validation_hourly_length = validation_day_length * 24
test_hourly_length = test_day_length * 24
train_hourly_length = len(target_hourly_data) - validation_hourly_length - test_hourly_length

target_train_hourly_data = target_hourly_data[0:train_hourly_length]
target_validation_hourly_data = target_hourly_data[train_hourly_length:train_hourly_length+validation_hourly_length]
target_test_hourly_data = target_hourly_data[train_hourly_length+validation_hourly_length:]
...
np.savetxt('./train/target_train_hourly_data.csv', target_train_hourly_data, fmt='%.4f', delimiter=',')
np.savetxt('./validation/target_validation_hourly_data.csv', target_validation_hourly_data, fmt='%.4f', delimiter=',')
np.savetxt('./test/target_test_hourly_data.csv', target_test_hourly_data, fmt='%.4f', delimiter=',')
